...
import { db, type Note, type Project, type VideoAsset } from '../db';
                          ...
                          .map((v) => (
                            ...
                            >
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <OffsetInput value={v.offset} onChange={(newOffset) => updateVideoOffset(v.id, newOffset)} />
        ...
        <div className="w-full landscape:w-64 md:w-72 border-t landscape:border-t-0 md:border-t-0 landscape:border-l md:border-l border-zinc-100 bg-white flex flex-col flex-1 landscape:flex-none md:flex-none min-h-0">
          <div className="p-4 border-b border-zinc-100 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold bg-zinc-100 px-2 py-0.5 rounded text-zinc-500">{notes.length}</span>
            </div>
          </div>
              ...
              <div className="p-8">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="flex items-center gap-2 text-zinc-500 bg-zinc-50 px-3 py-1.5 rounded-xl border border-zinc-100">
                    ...
                    <span className="text-sm font-mono font-bold">
                      {formatTimestamp(
                        editingNoteId ? notes.find((n) => n.id === editingNoteId)?.timestamp || currentTime : currentTime
                      )}
                    </span>
                  </div>
                </div>
  ...
  if (!project) {
    return (
      <div className="min-h-screen bg-zinc-50 flex flex-col items-center justify-center p-6 text-center">
        <div className="text-zinc-900 font-bold text-xl mb-2">Project not found</div>
        <div className="text-zinc-500 mb-6">This project may have been deleted.</div>
        <button
          onClick={() => navigate('/')}
          className="px-6 py-3 rounded-xl font-semibold bg-zinc-900 text-white hover:bg-zinc-800 transition-colors"
        >
          Back to Projects
        </button>
      </div>
