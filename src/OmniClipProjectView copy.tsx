...
const getTimelineDurationMS = (context: any) => {
  if (!context?.state?.effects?.length) return 0;
  ...
  useEffect(() => {
    ...
    const mountOmniClip = async () => {
      ...
      try {
        ...
        setDurationMS(getTimelineDurationMS(context));
      } finally {
              ...
              <span>{formatTimestampMS(durationMS)}</span>
		            ...
		            {omniContext && (
		              <OmniClipScrubber
		                ...
		                timecode={omniContext.state.timecode}
                  ...
                  sortedNotes.map((note) => (
                    ...
                    >
                      <div className="flex items-center gap-2 text-[11px] uppercase tracking-[0.18em] text-zinc-500 mb-2">
                        <StickyNote size={12} />
                        <span>{formatTimestampMS(note.timestamp)}</span>
                      </div>
